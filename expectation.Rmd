...
f~Y~|X=x (y|x) = f(x,y)/f~X~ (x) = (1-p)^2p^x/((1-p)^2(x+1)p^x) = 1/(x+1)
...
#### **Example** 
say X has a density f~X~ (x) = (a+1)x^a^, $0<x<1, a>1$.  $Y|X=x \sim$ Exp(x). Find E[Y] and Var(Y).
...
Let's have another look at the example of the "device" which generates a random number Y according to an exponential distribution with rate $\lambda$ where $\lambda=x$ with probability 0.5^x^ where x=1,2,3,... We previously found that f~Y~ (y) = 2e^y^/(2e^y^-1)^2, $y \ge 0$.
